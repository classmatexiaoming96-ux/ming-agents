...
> **示例**：§1~§6 各 doc_type 模板均含 `{{include_diagram}}` 占位 —— tech_review/design_doc 含 D-ARCH+D-SEQ（+D-CLASS），module_plan/research_report/task_plan/test_plan 含 D-DAG（v2.6 Phase 1 起补齐）。
>
...
> 说明：按 §0.4 D-DAG 骨架生成；节点 = 实施阶段，用 `subgraph Phase-N` 表达可并行阶段；边 = 阶段间前置依赖。节点名复用下表"阶段"列。
...
> 说明：按 §0.4 D-DAG 骨架生成；节点 ID 必须用上方"Task总览"表的 Task ID（如 TASK-001）；可并行的 Task 放入同一 `subgraph Phase-N`；边 = 依赖关系，须与下方"关键路径"一致。
...
> 说明：按 §0.4 D-DAG 骨架生成；节点 = 测试阶段（单元/集成/端到端…），用 `subgraph Phase-N` 表达可并行阶段；边 = 阶段间前置依赖。节点名复用下表"阶段"列。
