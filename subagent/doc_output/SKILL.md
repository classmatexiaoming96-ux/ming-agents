...
doc_output 是**纯 Python 脚本，自身不调用 LLM**。`template_engine` 只把模板里的 `{{include_diagram:D-XXX}}`（含 `{{include_diagram:multi}}` 与裸 `{{include_diagram}}`）占位替换成 **placeholder stub**（` ```mermaid ` 块内首行 `%% DOC_OUTPUT_DIAGRAM_PLACEHOLDER D-XXX`）—— 它**不是真图**。
...
**自检信号（双形式，v2.6 Phase 3 起）**：

1. **初始计数**：doc_output 返回契约里的 `unfilled_diagram_count` 字段反映**交付时**的图位待填数（几乎总是 > 0 —— stub 尚未填）；`gate_conditions.diagrams_filled` 同步反映此初始状态。
2. **填后 re-check**：调用方填完图位后，调用 `DocOutput.count_unfilled_diagrams(content)` 或 `DocOutput.recheck_unfilled_diagrams(path)` 重新核算 —— 这是真正的"完成门"，**必须返回 0** 才算图位齐备。Re-check 同时识别新 sentinel `%% DOC_OUTPUT_DIAGRAM_PLACEHOLDER` 和兼容旧版的 `%% PLACEHOLDER`。
...
*v2.6.0 | IdeaRefiner + 并行架构 + 飞书画板自动升级 + 图位 stub 调用方契约（含 re-check API）*
