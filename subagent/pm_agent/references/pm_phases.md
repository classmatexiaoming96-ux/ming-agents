...
**执行**：
1. 读取 PRD 内容
2. 通过 `sessions_spawn(runtime="acp", agentId="claude")` 调用 CC Lead
3. 在 prompt 中注入 `brainstorming` superpower（brainstorming 会自动做需求澄清 + 方案选型 + 用户 approval）
4. 等待 CC Lead 完成 brainstorming HARD GATE 流程（用户 approve 设计后 CC Lead 返回）
...
**⚠️ Known Limitation — subagent 无法执行 brainstorming**：

> **问题**：`sessions_spawn(runtime="acp")` 调起的 subagent **无法执行 askUserQuestion**（HARD GATE 的人机交互节点全部被跳过），导致 pm-n0a 无法收敛。
>
> **原因**：subagent 是隔离会话，没有通往用户的实时通道，askUserQuestion 会被静默跳过。
>
> **正确方式（已验证）**：
> 1. **不用 subagent 做 brainstorming** — 直接在**主会话**调用 CC Lead
> 2. 使用 `tmux` 创建独立 session：`tmux new -s shrimp-brainstorm`
> 3. 在 tmux 里执行 `claude -p`（或 `claude --acp --stdio`），传入 brainstorming superpower
> 4. brainstorming 通过终端输出向用户提问（不是 askUserQuestion），用户在 tmux 里回答
> 5. CC Lead 产出 `docs/idea_refinement.md` 后，主会话继续 pm-n0b
>
> **为什么不用 `sessions_spawn`**：sessions_spawn 的 subagent 传输层走 MCP 协议，askUserQuestion 不可用。tmux + claude -p 走 stdio，brainstorming superpower 可以正常输出交互式问题。
>
> **已验证可行流程**：
> ```bash
> tmux new -s shrimp-brainstorm -d
> tmux send-keys -t shrimp-brainstorm "cd {workspace_dir} && claude -p" Enter
> # 在 tmux 里手动运行 brainstorming...
> ```
...
**sessions_spawn 参数**（仅用于 pm-n1/cc_lead_writer，不用于 brainstorming）：
```json
{
  "task": "## 任务：Idea Refinement（需求澄清）\n\n读取 PRD 文档：{prd_path}\n\n请加载 **brainstorming** superpower 并执行需求澄清流程：\n1. 读取 PRD 内容\n2. 展示 2-3 个方案方向供用户选择\n3. 用户选择后深入设计\n4. 产出 `docs/idea_refinement.md`：\n   - 核心目标（1-3 个，可验证、有受益方）\n   - 约束条件（硬边界，标注已确认/推测）\n   - 开放问题（P0=阻塞立项，P1=阻塞方案，P2=可后续）\n\n**HARD GATE**：用户 approve 方案前不要进入任何实现阶段。",
  "runtime": "acp",
  "agentId": "claude",
  "mode": "session",
  "thread": true,
  ...
  "label": "pm-brainstorming-{requirement_id}"
}
```
...
3. 识别关键实体、输入输出、边界条件
4. 如发现新的歧义点，通过 `questions_for_user` 反馈给 Orchestrator
...
2. **并行**启动已启用的 Research Modules：
   - AIME Module：调用 AIME（sessions_send）
   - CC_Lead_Writer Module：调用 CC Lead 生成文档草稿（sessions_spawn acp）
...
7. **【自检】** 完成 SPEC.md 自检清单 + mermaid 自检清单
...
8. 为每个 sub-task 生成 failure_hypothesis
...
### 功能定位
CC_Lead_Writer 是 PM-Agent 的 Research Module 之一，通过 `sessions_spawn(runtime="acp", agentId="claude")` 调用本地的 CC Lead，让 CC Lead 基于实际代码仓库生成技术评审文档草稿。
...
3. **调用 CC Lead**：`sessions_spawn(runtime="acp", agentId="claude", task="<prompt>", label="pm-cc-lead-writer-{requirement_id}")`
4. **处理返回**：CC Lead 返回文档内容，写入 `docs/cc_lead_tech_review.md`
