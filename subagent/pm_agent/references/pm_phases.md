...
> **节点性质**：**HITL** —— brainstorming HARD GATE 强用户对话；见 `./afk_hitl.md`
...
> **节点性质**：**Mixed** —— 主体 AFK（解析 + bootstrap CONTEXT.md）；发现歧义 → `questions_for_user` 转 HITL
...
> **节点性质**：**AFK** —— 并行 Research Modules（AIME / CC_Lead_Writer / TechResearch / …），无用户交互；冲突写 `conflicts.md` 待用户裁决（在 Gate 1.5 处转 HITL）
...
> **节点性质**：**AFK** —— 汇总 + SPEC 生成 + ADR 记录 + 三套自检（Mermaid / SPEC / 统一语言），全程产物级自检；自检失败 → fail-with-hint / warning，必要时升级 HITL
...
> **节点性质**：**Mixed** —— PM↔Dev 自动迭代（最多 3 轮）；冲突需用户裁决时转 HITL
...
> **节点性质**：**AFK** —— 轻量评审 + 输出 planning_discussion.md
...
> **节点性质**：**AFK** —— Task Breakdown + 3-5 个可证伪 failure_hypothesis + rollback_plan + 依赖图，全自动
...
sub_tasks:
  {sub_task_id}:
    ...
    execution_mode: serial | parallel       # 能否与同级并行
    interaction_mode: afk | hitl | mixed    # 要不要等用户（与 execution_mode 正交），见 ./afk_hitl.md
    hours: float
...
> **节点性质**：**AFK** —— Dev-Agent 验 rollback 可行性，回 PM
...
> **节点性质**：**Mixed** —— PM↔Dev 协商 + dev_confirmed_rate ≥ 80% 后才进 DEV_PHASE；分歧/未达阈值时转 HITL
