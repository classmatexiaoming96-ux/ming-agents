...
每个 `subagent_dispatch` 节点 `interrupt` 取回 `result` 后,在落盘进 state 之前强制跑一遍 Anti-Drop Guard。失败结果记到 `state.warnings`(append-only),**首次 severe 触发一次带 hint 重试**(`GUARD_RETRY_LIMIT=1`),控制权送回同一节点;重试仍 severe 才把 stage 推到 BLOCKED。详见下方"失败动作"表。
...
- ✅ Anti-Drop Guard MVP(G1/G3 严重 + G5_PROVE 严重(文件不存在)/轻微(sha 不匹配)+ G4/G5_DECL/G6 轻微)在 6 个 dispatch 节点(pm/planner/dev_agent/codex_reviewer/rev_agent/codex_final)统一执行(包括 planning_dev_consult / codex_reviewer / codex_final 这三个只读 subagent —— 即使 `expected_artifacts=[]`,`_run_full_guard_check` 仍会反验任何 subagent 自报的 `artifact_updates`,堵住"声称写了文件但实际没写"的飘移路径);**首次 severe → 一次带 hint 重试**(详见下文"severe 失败带 hint 重试"条目),重试仍 severe 才 `state.stage=BLOCKED` + `state.warnings` 追加 + 后继边短路到 END;轻微失败 → 写 `warnings` 但正常推进。Hermes 必须在 severe warning 出现时当场 ⚠️
...
- ✅ Anti-Drop Guard severe 失败带 hint 重试:每个 `<stage>:<subagent>:r<retry_count>` 独立 `GUARD_RETRY_LIMIT=1` 一次重试机会;首次 severe → `state.pending_guard_retry` 写入 + 控制权送回同一节点 + dispatch_payload 挂 `guard_failure_hint`;再失败才 BLOCKED。DEV 内部 per-subTask 各自独立计数(`dev_agent[T-001]` / `codex_reviewer[T-001]` 等),T-001 用尽不影响 T-002 的额度;**rollback 后 `retry_count++` 也会让该 subagent 拿到新一轮 budget**(key 里 `r<retry_count>` 段变了),不会被上一轮的 guard_retries 计数挤掉重试机会。三种"重做"语义(rollback / 协商 / guard 重试)各自独立计数互不干扰
