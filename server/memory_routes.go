...
// D1: HTTP surface for the memory subsystem. Until now the memory package was
// reachable only through the memory-cli binary, so the daemon's agents could
// neither read nor write the "self-evolving memory" — Brief/Recall/Feedback/
// ImplicitFeedback had no production caller. These routes are thin adapters over
// the existing package-level functions (whose return values are already JSON
// ready); because every write flows through this single-process daemon, the
// daemon also serialises concurrent writers (complementing the atomic-write fix).
...
// registerMemoryRoutes attaches /api/memory/* to the mux.
func (s *Server) registerMemoryRoutes(mux *http.ServeMux) {
	mux.HandleFunc("POST /api/memory/ingest", s.handleMemoryIngest)
	mux.HandleFunc("GET /api/memory/recall", s.handleMemoryRecall)
	mux.HandleFunc("GET /api/memory/brief", s.handleMemoryBrief)
	mux.HandleFunc("POST /api/memory/feedback", s.handleMemoryFeedback)
	mux.HandleFunc("POST /api/memory/implicit", s.handleMemoryImplicit)
	mux.HandleFunc("POST /api/memory/cleanup", s.handleMemoryCleanup)
...
func (s *Server) handleMemoryIngest(w http.ResponseWriter, r *http.Request) {
	...
	if strings.TrimSpace(req.Content) == "" {
		writeErrorMsg(w, http.StatusBadRequest, "content is required")
		return
	}
...
func (s *Server) handleMemoryStats(w http.ResponseWriter, r *http.Request) {
	...
	writeJSON(w, http.StatusOK, map[string]any{
		...
		"superseded": superseded, "by_type": byType,
	})
}
