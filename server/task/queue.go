...
// back to pending so they can be re-claimed. Tasks that have reached max_attempts
// are marked as failed instead of being recovered. Returns the number recovered.
func (q *Queue) RecoverOrphanedTasks(ctx context.Context, timeout time.Duration) (int, error) {
	// First, mark tasks that have exceeded max_attempts as failed.
	_, err := q.pool.Exec(ctx, `
		UPDATE agent_task_queue
		SET status       = 'failed',
		    error        = 'max attempts exceeded during orphan recovery',
		    completed_at = now()
	...
	// Then recover remaining orphaned tasks back to pending.
