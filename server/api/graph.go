...
func (h *GraphHandler) handleAddNode(w http.ResponseWriter, r *http.Request) {
	...
	if node.RepoID == "" {
		writeAPIErrorMsg(w, http.StatusBadRequest, "repoID is required")
		return
	}
...
func (h *GraphHandler) handleGetReachable(w http.ResponseWriter, r *http.Request) {
	...
	if direction != "upstream" && direction != "downstream" {
		writeAPIErrorMsg(w, http.StatusBadRequest, "direction must be 'upstream' or 'downstream'")
		return
	}
	...
	if depthStr != "" {
		...
		if err != nil || maxDepth < 0 {
			writeAPIErrorMsg(w, http.StatusBadRequest, "depth must be a non-negative integer")
			return
		}
	}
