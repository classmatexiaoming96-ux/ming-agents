...
type ProcessPool struct {
	...
	registry *RepoGraph // optional registry for repo ID → path resolution
...
// SetRegistry sets the repo graph registry for repo ID → path resolution.
func (p *ProcessPool) SetRegistry(reg *RepoGraph) {
	p.mu.Lock()
	defer p.mu.Unlock()
	p.registry = reg
}
